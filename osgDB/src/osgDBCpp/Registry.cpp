...
#elif defined(WIN32)
    #ifdef _DEBUG
	    std::string strRes(prepend);
		strRes += "osgdb_";
		strRes += lowercase_ext;
		strRes += OSG_DEBUG_POSTFIX;
		strRes += ".dll";
        return strRes;
    #else
...
ReaderWriter::ReadResult Registry::read(const ReadFunctor& readFunctor)
{
        ...
        ++aitr)
    {
        ...
        if (positionArchive!=std::string::npos)
        {
            ...
            ReaderWriter::ReadResult result = openArchiveImplementation(archiveName,ReaderWriter::READ_, 4096, readFunctor._options);
