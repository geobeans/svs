...
#else // other unix
        osg::notify(osg::WARN)<<"~DynamicLibrary vxworks not support dynamic library"<<_name<<std::endl;
        //dlclose(_handle);
#endif    
    }
}
...
{
	osg::notify(osg::WARN)<<"DynamicLibrary loadLibrary: vxworks not support dynamic library"<<std::endl;
	return NULL;
...
{
    HANDLE handle = NULL;
    osg::notify(osg::WARN)<<"DynamicLibrary getLibraryHandle: vxworks not support dynamic library"<<std::endl;
//#if defined(WIN32) && !defined(__CYGWIN__)
...
{
    osg::notify(osg::WARN)<<"DynamicLibrary getProcAddress: vxworks not support dynamic library"<<_name<<std::endl;
    return NULL; 
